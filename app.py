...
st.write(f'Temperatura máxima: {leituras.TempOut.min()} °C')
st.write('---')
...
st.line_chart(data=leituras, x='Datetime', y='HumOut', color='#00FF00')
st.write(f'Umidade Mínima: {leituras.HumOut.min()} %')
st.write(f'Umidade Máxima: {leituras.HumOut.max()} %')
st.write('---')

st.subheader('Radiação Solar')
st.line_chart(data=leituras, x='Datetime', y='SolarRad', color='#FF8000')
